...
public class BackupService
{
    ...
    public BackupService(InkStudioDbContext db)
    {
        _db = db;
    }
    ...
    public static string ObtenerRutaCarpetaBackups()
    {
        ...
        var rutaBackups = Path.Combine(localAppData, "InkStudio", "backups");
    ...
    public static List<InfoServicioNube> DetectarServiciosNube()
    {
        ...
        {
            var backupPath = Path.Combine(oneDrivePath, "InkStudio", "Backups");
        ...
        {
            var backupPath = Path.Combine(googleDrivePath, "InkStudio", "Backups");
            servicios.Add(new InfoServicioNube
            {
        ...
        {
            var backupPath = Path.Combine(dropboxPath, "InkStudio", "Backups");
            servicios.Add(new InfoServicioNube
            {
    ...
    public async Task<string> CrearBackupAsync(string? rutaDestino = null)
    {
        try
        {
            ...
            var nombreArchivo = $"backup_inkstudio_{fecha:yyyy-MM-dd_HH-mm-ss-fff}.zip";
            ...
            var rutaDbTemporal = Path.Combine(Path.GetTempPath(), $"inkstudio_backup_{Guid.NewGuid()}.db");
    ...
    public static List<InfoBackup> ListarBackups(string carpeta)
    {
        ...
        var archivos = Directory.GetFiles(carpeta, "backup_inkstudio_*.zip");
...
Este archivo contiene un backup completo de InkStudio.

Para restaurar este backup en un PC nuevo:

1. Instalar InkStudio en el nuevo PC
2. Abrir InkStudio
